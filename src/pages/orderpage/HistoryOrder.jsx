...
export default function HistoryOrder() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className=" h-full overflow-y-scroll pl-3 pt-3 flex flex-col gap-1 flex-shrink  ">
        {historyOrders?.map((order) => (
          <div
            onClick={() => navigate(`/order/trackorder/${order.id}`)}
            key={order.id}
            className="border border-primary p-2 rounded-lg max-w-fit flex-shrink text-sm cursor-pointer mx-auto"
          >
